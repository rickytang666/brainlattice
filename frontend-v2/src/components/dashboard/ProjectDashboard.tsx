...
export default function ProjectDashboard() {
  ...
  const selectProject = (id: string) => {
    ...
    fetch(`${API_BASE}/project/${id}/graph`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleRename = async () => {
    if (!selectedProjectId || !newTitle.trim() || newTitle === currentProject?.title) {
        setEditingTitle(false);
        return;
    }
    
    try {
      const res = await fetch(`${API_BASE}/project/${selectedProjectId}/title`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: newTitle }),
      });
      if (res.ok) {
        setProjects(projects.map(p => p.id === selectedProjectId ? { ...p, title: newTitle } : p));
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="flex h-full w-full bg-[#0a0a0a] overflow-hidden text-neutral-200 font-sans">
      ...
      <div
        className={`flex flex-col border-r border-neutral-800 bg-[#0f0f0f] transition-all duration-300 ${selectedProjectId ? "w-64" : "w-full max-w-md mx-auto my-12 rounded-xl border border-neutral-800 shadow-2xl h-[calc(100vh-6rem)]"}`}
      >
            ...
            <div className="px-4 py-2 bg-neutral-900/80 backdrop-blur-md border border-neutral-800 rounded-full flex items-center gap-3">
              {editingTitle ? (
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setEditingTitle(false)} className="text-neutral-500 hover:text-red-400">
