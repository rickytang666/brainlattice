...
export const statusCommand = new Command('status')
  .description('check the status of your configuration and connection to the backend')
  .action(async () => {
    ...
    console.log(`- gemini_key: ${config.gemini_key ? chalk.green('configured') : chalk.red('missing')}`);
    console.log(`- openai_key: ${config.openai_key ? chalk.green('configured') : chalk.gray('optional')}`);
    ...
    try {
      ...
      if (!config.gemini_key) {
        console.log(chalk.yellow('\nwarning: gemini_key is missing. `gen` will fail until you add it.'));
      } else {
