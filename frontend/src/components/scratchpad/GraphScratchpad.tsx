...
export default function GraphScratchpad() {
  ...
  useEffect(() => {
    if (exampleId && EXAMPLES[exampleId]) {
      setGraphData(EXAMPLES[exampleId]);
    }
  }, [exampleId]);
  ...
  return (
    <div className="flex h-screen w-full bg-background overflow-hidden pt-[60px]">
      {/* Sidebar: JSON Input (Hidden for examples) */}
      {!exampleId && (
        <div className="w-1/3 flex flex-col border-r border-border bg-card">
          ...
          <div className="flex-1 p-4 relative flex flex-col">
            {mode === 'upload' ? (
              <div className="flex-1 border-2 border-dashed border-border rounded-lg flex flex-col items-center justify-center p-8 hover:border-foreground/30 hover:bg-muted/50 transition-all">
                <input 
                  type="file" 
                  ref={fileInputRef}
                ...
                >
                  <div className="p-4 bg-muted rounded-full transition-transform">
      ...
      <div className="flex-1 relative">
        {graphData ? (
          <>
            ...
            {/* Search Bar Overlay */}
            <div className="absolute top-6 left-1/2 -translate-x-1/2 z-10 w-full max-w-md">
              <SearchBar 
                data={graphData} 
                onSelectNode={(nodeId) => {
                  setFocusNodeId(nodeId);
        ...
        {/* Floating Back Button (for Example full screen views) */}
        {exampleId && (
          <Link 
            to="/" 
            className="absolute top-6 left-6 flex items-center gap-2 px-4 py-2 bg-popover text-muted-foreground hover:text-foreground border border-border rounded-full shadow-sm hover:shadow transition-all"
          >
            <ArrowLeft className="w-4 h-4" />
